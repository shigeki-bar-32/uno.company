...
$(function() {
    ...
    function filterItems() {
      var filter = $('.filter-buttons button.active').data('filter');
      ...
      $items.each(function() {
        ...
        var show = (filter === 'all' || filter === category) && title.indexOf(keyword) !== -1;
        $this.toggle(show);
      });
    }
  });
...
//   header catchphraseの設定
$(function() {
    // ページ読み込み時に現在のURLパスを取得
    var path = window.location.pathname.split('/').pop();
  
    // pathが空文字かindex.htmlならホームページと判断して表示
